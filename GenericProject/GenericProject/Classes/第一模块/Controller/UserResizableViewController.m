...
@interface UserResizableViewController ()<LSUserResizableViewDelegate> {
...
@implementation UserResizableViewController
 ...
 */
- (void)setupTopView {
    _topScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, LSScreenWidth, LSScreenHeight/2 + 20)];
    _topScrollView.autoresizesSubviews = YES;
    [self.view addSubview:_topScrollView];
    
    _label = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, LSScreenWidth, LSScreenHeight)];
    ...
    _label.backgroundColor = LSWhiteColor;
    [_topScrollView addSubview:_label];
    
    CGSize maxSize = CGSizeMake(LSScreenWidth, MAXFLOAT);
...
- (void)setupResizableView {
    ...
    CGRect gripFrame = CGRectMake(0, LSScreenHeight/2, LSScreenWidth, LSScreenHeight/2);
    LSUserResizableView *userResizableView = [[LSUserResizableView alloc] initWithFrame:gripFrame];
    ...
    CGSize maxSize = CGSizeMake(LSScreenWidth, MAXFLOAT);
    CGFloat textHeight = [_label.text boundingRectWithSize:maxSize options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:17]} context:nil].size.height;
    userResizableView.bottomDistance = LSScreenHeight - textHeight > 0 ? 50.0f : 50.0f + LSScreenHeight - textHeight;
    
    // 2.文字dragBtn（最底层，只是用来显示文字而已）
    UIButton *dragBtn = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, LSScreenWidth, 24)];
    ...
    UIScrollView *scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 20, LSScreenWidth, LSScreenHeight/2 - 20)];
    scrollView.contentSize = CGSizeMake(LSScreenWidth * 3, 0);
    ...
    for (int i = 0; i < 3; i++) {
        ...
        // 4.1.2文字
        UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, LSScreenWidth, 40)];
...
- (void)userResizableViewDidBeginEditing:(LSUserResizableView *)userResizableView {
    ...
    scrollFrame.size.height = LSScreenHeight - CGRectGetHeight(userResizableView.frame) + 20;
