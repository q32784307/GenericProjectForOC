...
#import "UIView+LSViewGradient.h"

@implementation UIView (LSViewGradient)
...
+ (void)load {
    ...
    for (NSString *str in arr) {
        NSString *new_str = [@"ls_" stringByAppendingString:str];
...
- (void)ls_setFrame:(CGRect)frame {
    [self ls_setFrame:frame];
    [self refreshLayout];
}
...
- (void)ls_removeFromSuperview {
    ...
    [self ls_removeFromSuperview];
}

- (void)ls_setAlpha:(CGFloat)alpha {
    [self ls_setAlpha:alpha];
...
- (void)ls_setHidden:(BOOL)hidden {
    [self ls_setHidden:hidden];
    if (self.shadowView) {
