//
//  UILabel+LSAttributeTextTapAction.m
//  EasyPartTime
//
...
#import "UILabel+LSAttributeTextTapAction.h"

@interface LSAttributeModel : NSObject
...
@implementation LSAttributeModel

@end

@implementation UILabel (LSAttributeTextTapAction)
...
- (id<LSAttributeTapActionDelegate>)delegate {
    return objc_getAssociatedObject(self, _cmd);
}

- (void)setDelegate:(id<LSAttributeTapActionDelegate>)delegate {
...
- (void)ls_addAttributeTapActionWithStrings:(NSArray <NSString *> *)strings tapClicked:(void (^) (UILabel * label, NSString *string, NSRange range, NSInteger index))tapClick {
    [self ls_removeAttributeTapActions];
    [self ls_getRangesWithStrings:strings];
...
- (void)ls_addAttributeTapActionWithRanges:(NSArray<NSString *> *)ranges tapClicked:(void (^)(UILabel *, NSString *, NSRange, NSInteger))tapClick {
    [self ls_removeAttributeTapActions];
    [self ls_getRangesWithRanges:ranges];
    self.userInteractionEnabled = YES;
...
- (void)ls_addAttributeTapActionWithRanges:(NSArray<NSString *> *)ranges delegate:(id<LSAttributeTapActionDelegate>)delegate {
    [self ls_removeAttributeTapActions];
    [self ls_getRangesWithRanges:ranges];
    self.userInteractionEnabled = YES;
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    BOOL ret = [self ls_getTapFrameWithTouchPoint:point result:^(NSString *string, NSRange range, NSInteger index) {
        
        if (weakSelf.isTapEffect) {
            
            [weakSelf ls_saveEffectDicWithRange:range];
            
            [weakSelf ls_tapEffectWithStatus:YES];
        }
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    if (self.isTapEffect) {
        [self performSelectorOnMainThread:@selector(ls_tapEffectWithStatus:) withObject:nil waitUntilDone:NO];
    }
    ...
    BOOL ret = [self ls_getTapFrameWithTouchPoint:point result:^(NSString *string, NSRange range, NSInteger index) {
        ...
        if (weakSelf.delegate && [weakSelf.delegate respondsToSelector:@selector(ls_tapAttributeInLabel:string:range:index:)]) {
            [weakSelf.delegate ls_tapAttributeInLabel:weakSelf string:string range:range index:index];
        }
    }];
    if (!ret) {
        [super touchesEnded:touches withEvent:event];
...
- (BOOL)ls_getTapFrameWithTouchPoint:(CGPoint)point result:(void (^) (NSString *string , NSRange range , NSInteger index))resultBlock {
    ...
    CGFloat total_height =  [self ls_textSizeWithAttributedString:self.attributedText width:self.bounds.size.width numberOfLines:0].height;
    ...
    CGAffineTransform transform = [self ls_transformForCoreText];
    ...
    for (CFIndex i = 0; i < count; i++) {
        ...
        CGRect flippedRect = [self ls_getLineBounds:line point:linePoint];
        
        CGRect rect = CGRectApplyAffineTransform(flippedRect, transform);
        
        CGFloat lineOutSpace = (self.bounds.size.height - total_height) / 2;
        
        rect.origin.y = lineOutSpace + [self ls_getLineOrign:line];
        ...
        if (CGRectContainsPoint(rect, point)) {
            ...
            for (int j = 0; j < link_count; j++) {
                ...
                LSAttributeModel *model = self.attributeStrings[j];
...
- (CGAffineTransform)ls_transformForCoreText {
    return CGAffineTransformScale(CGAffineTransformMakeTranslation(0, self.bounds.size.height), 1.f, -1.f);
}

- (CGRect)ls_getLineBounds:(CTLineRef)line point:(CGPoint)point {
    CGFloat ascent = 0.0f;
    ...
    height = [self ls_textSizeWithAttributedString:attributedString width:self.bounds.size.width numberOfLines:0].height;
    return CGRectMake(point.x, point.y , width, height);
}

- (CGFloat)ls_getLineOrign:(CTLineRef)line {
    CFRange range = CTLineGetStringRange(line);
    if (range.location == 0) {
        return 0.;
    }else {
        ...
        return [self ls_textSizeWithAttributedString:attributedString width:self.bounds.size.width numberOfLines:0].height;
    }
}

- (CGSize)ls_textSizeWithAttributedString:(NSAttributedString *)attributedString width:(float)width numberOfLines:(NSInteger)numberOfLines {
    @autoreleasepool {
...
- (void)ls_tapEffectWithStatus:(BOOL)status {
    if (self.isTapEffect) {
...
- (void)ls_saveEffectDicWithRange:(NSRange)range {
...
- (void)ls_getRangesWithStrings:(NSArray <NSString *>  *)strings {
    ...
    [strings enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (range.length != 0) {
            ...
            totalStr = [totalStr stringByReplacingCharactersInRange:range withString:[weakSelf ls_getStringWithRange:range]];
            
            LSAttributeModel *model = [LSAttributeModel new];
            model.range = range;
            model.str = obj;
...
- (void)ls_getRangesWithRanges:(NSArray <NSString *>  *)ranges {
    if (self.attributedText == nil) {
        self.isTapAction = NO;
        return;
    }
...
- (NSString *)ls_getStringWithRange:(NSRange)range {
