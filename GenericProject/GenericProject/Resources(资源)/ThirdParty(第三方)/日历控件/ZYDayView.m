...
- (void)changeState {
    ...
    // 非空dayView的状态
        ...
        if ([_manager.helper date:_date isTheSameDayThan:_manager.date] && self.enabled) {
            [self setImage:[[UIImage imageNamed:@""] imageWithRenderingMode:_manager.imageRenderingMode] forState:UIControlStateNormal];
        } else {
            [self setImage:nil forState:UIControlStateNormal];
        }
        ...
        // 范围选择
            ...
            if (_manager.selectedDateArray.count == 2) {
                ...
                // 设置起始按钮背景图片
                if ([_manager.helper date:_date isTheSameDayThan:_manager.selectedDateArray[0]]) {
                    [self setBackgroundImage:[[UIImage imageNamed:@"backImg_start"] imageWithRenderingMode:_manager.imageRenderingMode] forState:UIControlStateSelected];
                    self.selected = true;
                } else if ([_manager.helper date:_date isTheSameDayThan:_manager.selectedDateArray[1]]) {
                    [self setBackgroundImage:[[UIImage imageNamed:@"backImg_end"] imageWithRenderingMode:_manager.imageRenderingMode] forState:UIControlStateSelected];
                    self.selected = true;
                } else {
                    [self setBackgroundImage:nil forState:UIControlStateSelected];
                }
