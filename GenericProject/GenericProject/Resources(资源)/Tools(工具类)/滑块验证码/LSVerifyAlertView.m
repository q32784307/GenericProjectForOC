...
- (void)setupContentView {
    ...
    [_contentView addSubview:({
        _puzzleView = [[LSPuzzleVerifyView alloc] init];
...
- (void)performCallback {
    if (!_isVerified) {
        if (_remainingVerifyNumber == 0) {
            _state = LSVerifyStateFail; // 验证失败
        } else if (_remainingVerifyNumber == _maximumVerifyNumber)  {
            _state = LSVerifyStateNot; // 未开始验证
        } else {
            _state = LSVerifyStateIncomplete; // 未完成验证
        }
    } else {
        _state = LSVerifyStateSuccess; // 验证成功
    }
