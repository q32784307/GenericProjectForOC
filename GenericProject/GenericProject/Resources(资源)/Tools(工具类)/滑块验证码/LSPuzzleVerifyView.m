...
#import "LSPuzzleVerifyView.h"
#import "UIBezierPath+LSPuzzlePathMaker.h"

@interface LSPuzzleVerifyView ()
...
@implementation LSPuzzleVerifyView

#pragma mark --  Init
- (instancetype)init {
    return [self initWithFrame:CGRectZero style:LSPuzzleVerifyStyleClassic];
}

- (instancetype)initWithFrame:(CGRect)frame {
    return [self initWithFrame:frame style:LSPuzzleVerifyStyleClassic];
}

- (instancetype)initWithFrame:(CGRect)frame style:(LSPuzzleVerifyStyle)style {
...
- (void)initWithStyle:(LSPuzzleVerifyStyle)style {
...
- (void)setStyle:(LSPuzzleVerifyStyle)style {
    _style = style;
    
    if (style == LSPuzzleVerifyStyleCustom) return;
...
- (void)setPuzzlePath:(UIBezierPath *)puzzlePath {
    ...
    if (_style != LSPuzzleVerifyStyleCustom) return;
...
- (UIBezierPath *)pathForStyle:(LSPuzzleVerifyStyle)style {
    switch (style) {
        case LSPuzzleVerifyStyleClassic:
            return [self classicPuzzlePath];
        case LSPuzzleVerifyStyleSquare:
            return [self squarePuzzlePath];
        case LSPuzzleVerifyStyleCircle:
            return [self circlePuzzlePath];
        case LSPuzzleVerifyStyleCustom:
            return nil;
    }
}
...
- (UIBezierPath *)classicPuzzlePath {
    ...
    dispatch_once(&onceToken, ^{
        classicPath = [UIBezierPath bezierPathWithPathMaker:^(LSPuzzlePathMaker * _Nonnull maker) {
