...
#import "UIView+LSImage.h"
#import "LSBlurView.h"
#import "LSEditMenuView.h"
#import "LSEditSelectedBox.h"
#import "LSImage.h"
#import "LSImageView.h"
#import "LSDrawView.h"
#import "LSEditTextView.h"
#import "LSMosaicView.h"
#import "LSImageZoomView.h"
#import "LSDelayPerform.h"
#import "UIImage+LSCommon.h"
#import "LSImageClipViewController.h"

@interface LSEditImageViewController ()<UIGestureRecognizerDelegate, LSImageZoomViewDelegate>

@property(nonatomic,strong)LSImageZoomView *zoomView; // 预览视图 展示编辑的图片 可以缩放

@property(nonatomic,strong)LSBlurView *editBtn; //编辑
@property(nonatomic,strong)LSBlurView *againShotBtn;  // 再拍一次
...
@property(nonatomic,strong)LSEditMenuView *editMenuView; //编辑菜单栏
@property(nonatomic,strong)UIButton *trashTips; //垃圾桶提示 拖拽删除 贴图或文字

@property(nonatomic,strong)LSDrawView *drawView; // 涂鸦视图
@property(nonatomic,strong)NSMutableArray *watermarkArray; // 水印层 所有的贴图和文本
@property(nonatomic,strong)LSEditSelectedBox *selectedBox; //水印选中框
@property(nonatomic,strong)LSMosaicView *mosaicView; //马赛克画板

@property(nonatomic,assign)LSEditMenuType editingMenuType; //当前正在编辑的菜单类型
...
- (void)setupUI {
    ...
    if (self.image.size.width > 0) {
       self.zoomView.imageView.frame = CGRectMake(0, 0, self.zoomView.width, self.zoomView.width * self.image.size.height / self.image.size.width);
    }
    if (self.zoomView.imageView.height <= self.zoomView.height) {
        self.zoomView.imageView.center = CGPointMake(self.zoomView.width / 2.0, self.zoomView.height / 2.0);
    }
...
- (void)topSelectedView:(UIView *)topView {
    ...
    [LSDelayPerform ls_cancelDelayPerform]; //取消延迟执行
...
#pragma mark - Setter
- (void)setEditingMenuType:(LSEditMenuType)editingMenuType {
    _editingMenuType = editingMenuType;
    switch (_editingMenuType) {
        case LSEditMenuTypeUnknown:
            self.zoomView.scrollEnabled = YES;
            self.zoomView.pinchGestureRecognizer.enabled = YES;
            break;
        case LSEditMenuTypeGraffiti:
            self.zoomView.pinchGestureRecognizer.enabled = YES;
            self.zoomView.scrollEnabled = NO;
            break;
        case LSEditMenuTypeText:
            self.zoomView.scrollEnabled = YES;
            self.zoomView.pinchGestureRecognizer.enabled = NO;
            break;
        case LSEditMenuTypeSticking:
            self.zoomView.scrollEnabled = YES;
            self.zoomView.pinchGestureRecognizer.enabled = NO;
            break;
        case LSEditMenuTypePictureMosaic:
            self.zoomView.scrollEnabled = NO;
            self.zoomView.pinchGestureRecognizer.enabled = YES;
            break;
        case LSEditMenuTypePictureClipping:
            self.zoomView.scrollEnabled = YES;
            self.zoomView.pinchGestureRecognizer.enabled = YES;
            break;
        default:
...
#pragma mark - Getter
- (LSImageZoomView *)zoomView {
    if (_zoomView == nil) {
        _zoomView = [[LSImageZoomView alloc] initWithFrame:self.view.bounds];
...
- (LSBlurView *)editBtn {
    if (_editBtn == nil) {
        _editBtn = [[LSBlurView alloc] initWithFrame:CGRectMake(0, 0, 70, 70)];
        _editBtn.center = CGPointMake(self.view.width / 2.0, self.view.height - 80);
        _editBtn.layer.cornerRadius = _editBtn.width / 2.0;
...
- (LSBlurView *)againShotBtn {
    if (_againShotBtn == nil) {
        _againShotBtn = [[LSBlurView alloc] initWithFrame:CGRectMake(0, 0, 70, 70)];
        _againShotBtn.center = CGPointMake((self.view.width / 2 - 70 / 2.0) / 2.0, self.view.height - 80);
        _againShotBtn.layer.cornerRadius = _againShotBtn.width / 2.0;
...
- (UIButton *)saveAlbumBtn {
    if (_saveAlbumBtn == nil) {
        ...
        _saveAlbumBtn.center = CGPointMake(self.view.width / 2.0 + 70 / 2.0 + (self.view.width / 2 - 70 / 2.0) / 2.0, self.view.height - 80);
        _saveAlbumBtn.layer.cornerRadius = _saveAlbumBtn.width / 2.0;
...
- (UIButton *)doneEditBtn {
    if (_doneEditBtn == nil) {
        _doneEditBtn = [[UIButton alloc] initWithFrame:CGRectMake(self.view.width - 50 - 15, 30, 40, 30)];
...
- (LSEditMenuView *)editMenuView {
    if (!_editMenuView) {
        _editMenuView = [[LSEditMenuView alloc] initWithFrame:CGRectMake(0, self.view.height - 80 -  60, self.view.width, 80 + 60)];
        _editMenuView.hidden = YES;
        __weak typeof(self) weakSelf = self;
        _editMenuView.editObject = LSEditObjectPicture;
        _editMenuView.selectEditMenu = ^(LSEditMenuType editMenuType, NSDictionary * _Nullable setting) {
            weakSelf.editingMenuType = ![setting[@"hidden"] boolValue] ? editMenuType : LSEditMenuTypeUnknown;
            if (editMenuType == LSEditMenuTypeGraffiti) {
            ...
            if (editMenuType == LSEditMenuTypeSticking) {
                LSImage *image = setting[@"image"];
                if ([setting[@"hidden"] boolValue]) weakSelf.editingMenuType = LSEditMenuTypeUnknown;
                if (image) {
                    LSImageView *imageView = [[LSImageView alloc] initWithFrame:CGRectMake(0, 0, image.size.width / [UIScreen mainScreen].scale, image.size.height / [UIScreen mainScreen].scale)];
                    imageView.autoPlayAnimatedImage = NO;
                    imageView.userInteractionEnabled = YES;
                    CGRect imageRect = [weakSelf.zoomView convertRect:weakSelf.zoomView.imageView.frame toView:weakSelf.view];
                    CGPoint center = CGPointZero;
                    center.x = fabs(imageRect.origin.x) + weakSelf.zoomView.width / 2.0;
                    center.y = 0;
                    if (imageRect.origin.y >= 0 && imageRect.size.height <= weakSelf.zoomView.height) {
                        center.y = imageRect.size.height/2.0;
                    }else {
                        center.y = fabs(imageRect.origin.y) + weakSelf.zoomView.height / 2.0;
                    }
            ...
            if(editMenuType == LSEditMenuTypePictureMosaic) {
            ...
            if (editMenuType == LSEditMenuTypePictureClipping) {
                LSImageClipViewController *imageClipController = [[LSImageClipViewController alloc] init];
                imageClipController.modalPresentationStyle = UIModalPresentationFullScreen;
                [weakSelf.selectedBox removeFromSuperview];
                UIImage *image = [weakSelf.zoomView.imageView ls_imageByViewInRect:weakSelf.zoomView.imageView.bounds];
...
- (LSDrawView *)drawView {
    if (!_drawView) {
        _drawView = [[LSDrawView alloc] initWithFrame:self.zoomView.imageView.bounds];
...
- (LSEditSelectedBox *)selectedBox {
    if (!_selectedBox) {
        _selectedBox = [[LSEditSelectedBox alloc] init];
    }
    return _selectedBox;
}

- (LSMosaicView *)mosaicView {
    if (!_mosaicView) {
        _mosaicView = [[LSMosaicView alloc] initWithFrame:self.zoomView.imageView.bounds];
        __weak typeof(self) weakSelf = self;
        ...
        _mosaicView.brushColor = ^UIColor *(CGPoint point) {
            ...
            return [weakSelf.zoomView.image ls_colorAtPixel:point];
        };
...
- (void)cancleEditBtnClicked:(id)sender {
    ...
    self.zoomView.contentSize = CGSizeMake(self.zoomView.imageView.width, self.zoomView.imageView.height);
}

//完成编辑 导出编辑后的对象
- (void)doneEditBtnClicked:(id)sender {
    [self.selectedBox removeFromSuperview];
    self.image = [self.zoomView.imageView ls_imageByViewInRect:self.zoomView.imageView.bounds];
...
- (void)dragAction:(UIPanGestureRecognizer *)pan {
    ...
    if (pan.state == UIGestureRecognizerStateBegan) {
        ...
        if (self.trashTips.center.y < rect.origin.y+rect.size.height / 2.0) {
            ...
            [self.watermarkArray removeObject:(LSImageView *)pan.view];
        }else if (!CGRectIntersectsRect(imageRect, rect)) {
            //如果出了父视图zoomView的范围，则置于父视图中心
            CGPoint center = CGPointZero;
            center.x = fabs(imageRect.origin.x) + self.zoomView.width / 2.0;
            center.y = 0;
            if (imageRect.origin.y >= 0 && imageRect.size.height <= self.zoomView.height) {
                center.y = imageRect.size.height / 2.0;
            }else {
                center.y = fabs(imageRect.origin.y) + self.zoomView.height / 2.0;
            }
...
- (void)imageClippingComplete:(NSNotification *)notification {
    ...
    self.zoomView.imageView.frame = CGRectMake(0, 0, self.zoomView.width, self.zoomView.width * clipImage.size.height / clipImage.size.width);
    if (self.zoomView.imageView.height <= self.zoomView.height) {
        self.zoomView.imageView.center = CGPointMake(self.zoomView.width / 2.0, self.zoomView.height / 2.0);
    }
    self.zoomView.contentSize = CGSizeMake(self.zoomView.imageView.width, self.zoomView.imageView.height);
