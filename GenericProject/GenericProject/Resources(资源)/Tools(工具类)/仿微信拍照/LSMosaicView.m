...
/// 马赛克点元素
@interface LSMosaicPointElement : NSObject
...
@implementation LSMosaicPointElement

@end

/// 马赛克线条 遮罩层  线
@interface LSMosaicLineLayer : CALayer

@property(nonatomic,strong)NSMutableArray <LSMosaicPointElement *>*elementArray;

@end

@implementation LSMosaicLineLayer
...
- (void)drawInContext:(CGContextRef)context {
    ...
    for (NSInteger i=0; i<self.elementArray.count; i++) {
        LSMosaicPointElement *blur = self.elementArray[i];
...
/// 马赛克画板
@interface LSMosaicView ()
{
    BOOL _isWork;
    BOOL _isBegan;
}
/** 图层 */
@property(nonatomic,strong)NSMutableArray <LSMosaicLineLayer *>*layerArray;
/** 已显示坐标 */
@property(nonatomic,strong)NSMutableArray <NSValue *>*frameArray;

@end

@implementation LSMosaicView
...
- (void)customInit {
    ...
    _mosaicType = LSMosaicTypeSquare;
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    if (touches.allObjects.count == 1) {
        ...
        //2、创建LFSplashBlur
        if (self.mosaicType == LSMosaicTypeSquare) {
            ...
            if (![self.frameArray containsObject:value]) {
                ...
                LSMosaicPointElement *blur = [LSMosaicPointElement new];
                ...
                LSMosaicLineLayer *layer = [LSMosaicLineLayer layer];
        ...
        } else if (self.mosaicType == LSMosaicTypePaintbrush) {
            LSMosaicPointElement  *blur = [LSMosaicPointElement  new];
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
    if (_isBegan || _isWork) {
        ...
        /** 获取上一个对象坐标判断是否重叠 */
        LSMosaicLineLayer *layer = self.layerArray.lastObject;
        LSMosaicPointElement *prevBlur = layer.elementArray.lastObject;
...
- (void)goBack {
    ...
    LSMosaicLineLayer *layer = self.layerArray.lastObject;
    if ([layer.elementArray.firstObject isMemberOfClass:[LSMosaicPointElement class]]) {
        for (LSMosaicPointElement *blur in layer.elementArray) {
...
- (NSDictionary *)data {
    if (self.layerArray.count) {
        ...
        for (LSMosaicLineLayer *layer in self.layerArray) {
