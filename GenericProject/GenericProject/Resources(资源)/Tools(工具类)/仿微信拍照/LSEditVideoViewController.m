...
#import "LSEditVideoViewController.h"
#import "LSAVPlayer.h"
#import "LSBlurView.h"
#import "LSEditMenuView.h"
#import "LSDrawView.h"
#import "LSEditSelectedBox.h"
#import "LSEditVideoClipping.h"
#import "LSDelayPerform.h"
#import "UIView+LSImage.h"
#import "LSImage.h"
#import "LSImageView.h"
#import "LSEditTextView.h"
#import "LSAVEditExport.h"

@interface LSEditVideoViewController ()<UIGestureRecognizerDelegate, LSAVPlayerDelegate>

@property(nonatomic,strong)UIImageView *preview; // 预览视图 展示编辑的图片或视频
@property(nonatomic,strong)LSAVPlayer *avPlayer;  //视频播放预览
...
@property(nonatomic,strong)LSEditVideoClipping *videoClippingView; //视频裁剪 子菜单视图 选择裁剪范围
@property(nonatomic,assign)CMTime clippingBeginTime; //视频裁剪起始点
@property(nonatomic,assign)CMTime clippingEndTime; //视频裁剪结束点

@end

@implementation LSEditVideoViewController
...
- (void)setupUI {
    ...
    if (self.avPlayer.naturalSize.width != CGSizeZero.width) {
         self.preview.height = self.preview.width *  self.avPlayer.naturalSize.height / self.avPlayer.naturalSize.width;
    }
    self.avPlayer.monitor = self.preview;
    self.preview.center = CGPointMake(self.view.width / 2.0, self.view.height / 2.0);
    [self.avPlayer play];
...
- (CALayer *)graffitiLayer {
    ...
    CGSize scaleSize = CGSizeMake(self.avPlayer.naturalSize.width / self.preview.width, self.avPlayer.naturalSize.height / self.preview.height);
    ...
    UIImage *image = [self.drawView ls_imageByViewInRect:self.drawView.bounds];
    /** 缩放至视频大小 */
...
- (NSMutableArray *)watermarkLayers {
    ...
    for (UIView *view in self.watermarkArray) {
        ...
        animatedLayer.position =  CGPointMake(view.center.x*scaleSize.width, (self.preview.height - view.center.y)*scaleSize.height);
        ...
        if ([view isKindOfClass:[LSImageView class]]) {
            LSImageView *imageView = (LSImageView *)view;
            if (imageView.imageType == LSImageTypeGIF) {
            ...
            LSImage *image = [LSImage imageWithData:UIImagePNGRepresentation([label ls_imageByViewInRect:label.bounds])];
            animatedLayer.contentsScale = [UIScreen mainScreen].scale;
...
// Gif CALayer关键帧动画
- (CAKeyframeAnimation *)animationForGifWithImage:(LSImage *)image {
...
- (LSAVPlayer *)avPlayer {
    if (!_avPlayer) {
        _avPlayer = [[LSAVPlayer alloc] init];
    }
    return _avPlayer;
}
...
- (LSEditMenuView *)editMenuView {
    if (!_editMenuView) {
        ...
        _editMenuView.editObject = LSEditObjectVideo;
        ...
        _editMenuView.selectEditMenu = ^(LSEditMenuType editMenuType, NSDictionary * _Nullable setting) {
            ...
            if (editMenuType == LSEditMenuTypeSticking) {
                ...
                if (image) {
                    ...
                    imageView.center = CGPointMake(weakSelf.preview.width / 2.0, weakSelf.preview.height / 2.0);
            ...
            if (editMenuType == LSEditMenuTypeText) {
                ...
                editTextView.editTextCompleted = ^(UILabel * _Nullable label) {
                    ...
                    label.center = CGPointMake(weakSelf.preview.width / 2.0, weakSelf.preview.height / 2.0);
            ...
            if(editMenuType == LSEditMenuTypeVideoClipping) {
...
- (LSDrawView *)drawView {
    if (!_drawView) {
        _drawView = [[LSDrawView alloc] initWithFrame:self.preview.bounds];
...
- (void)exportEditVideo {
    ...
    LSAVEditExport *videoExportSession = [[LSAVEditExport alloc] initWithAsset:[AVAsset assetWithURL:self.videoPath]];
...
- (void)dragAction:(UIPanGestureRecognizer *)pan {
    ...
    if (pan.state == UIGestureRecognizerStateBegan) {
        ...
        if (self.trashTips.center.y < rect.origin.y+rect.size.height/2.0) {
            ...
            pan.view.center = CGPointMake(self.preview.width / 2.0, self.preview.height / 2.0);
        }
