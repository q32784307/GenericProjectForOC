...
@interface LSShotViewController ()<LSAVCaptureToolDelegate>
{
    dispatch_source_t _gcdTimer; //计时器
    NSTimeInterval _durationOfVideo;  //录制视频的时长
}

@property(nonatomic,strong)LSAVCaptureTool *avCaptureTool; //摄像头采集工具
...
@property(nonatomic,strong)LSBlurView *shotBtn; //拍摄按钮
...
@property(nonatomic,strong)LSShotFocusView *focusView;   //当前聚焦视图

@end

@implementation LSShotViewController
...
- (void)viewDidDisappear:(BOOL)animated {
    ...
    [LSDelayPerform ls_cancelDelayPerform];
}
...
#pragma mark - Getter
- (LSAVCaptureTool *)avCaptureTool {
    if (_avCaptureTool == nil) {
        _avCaptureTool = [[LSAVCaptureTool alloc] init];
...
- (UIButton *)backBtn {
    if (_backBtn == nil) {
        ...
        _backBtn.center = CGPointMake((self.view.width / 2 - 70 / 2.0) / 2.0, self.view.height - 80);
...
- (UIView *)shotBtn {
    if (_shotBtn == nil) {
        _shotBtn = [[LSBlurView alloc] init];
        _shotBtn.userInteractionEnabled = YES;
        _shotBtn.frame = CGRectMake(0, 0, 70, 70);
        _shotBtn.center = CGPointMake(self.view.width / 2.0, self.view.height - 80);
        _shotBtn.clipsToBounds = YES;
        _shotBtn.layer.cornerRadius = _shotBtn.width / 2.0;
        //轻触拍照，长按摄像
        ...
        self.whiteView.center = CGPointMake(_shotBtn.width / 2.0, _shotBtn.height / 2.0);
...
- (UIButton *)switchCameraBtn {
    if (_switchCameraBtn == nil) {
        _switchCameraBtn = [[UIButton alloc] initWithFrame:CGRectMake(self.view.width - 30 - 30, 44 , 30, 30)];
...
- (LSShotFocusView *)focusView {
    if (_focusView == nil) {
        _focusView= [[LSShotFocusView alloc] initWithFrame:CGRectMake(0, 0, 60, 60)];
    }
    return _focusView;
}
...
- (UILabel *)tipsLabel {
    if (_tipsLabel == nil) {
        _tipsLabel = [[UILabel alloc] initWithFrame:CGRectMake((self.view.width - 140)/2.0, self.shotBtn.y - 20 - 30, 140, 20)];
...
- (void)tapFocusing:(UITapGestureRecognizer *)tap {
    ...
    if(point.y > self.shotBtn.y || point.y < self.switchCameraBtn.y + self.switchCameraBtn.height) {
        return;
    }
...
- (void)recordVideo:(UILongPressGestureRecognizer *)longPress {
    switch (longPress.state) {
        case UIGestureRecognizerStateBegan:{
            self.shotBtn.size = CGSizeMake(100, 100);
            self.shotBtn.center = CGPointMake(self.view.width / 2.0, self.view.height - 80);
            self.shotBtn.layer.cornerRadius =  self.shotBtn.height / 2.0;
            self.whiteView.size = CGSizeMake(40, 40);
            self.whiteView.center = CGPointMake(self.shotBtn.width / 2.0, self.shotBtn.height / 2.0);
            self.whiteView.layer.cornerRadius = self.whiteView.width / 2.0;
            //开始计时
            [self startTimer];
            //添加进度条
            ...
            //开始录制视频
            [self.avCaptureTool startRecordVideoToOutputFileAtPath:outputVideoFielPath recordType:LSAVCaptureTypeAv];
        }
            NSLog(@"开始摄像");
            break;
        ...
        case UIGestureRecognizerStateEnded:{
            self.shotBtn.size = CGSizeMake(70, 70);
            self.shotBtn.center = CGPointMake(self.view.width / 2.0, self.view.height - 80);
            self.shotBtn.layer.cornerRadius =  self.shotBtn.height / 2.0;
            self.whiteView.size = CGSizeMake(50, 50);
            self.whiteView.center = CGPointMake(self.shotBtn.width / 2.0, self.shotBtn.height / 2.0);
            self.whiteView.layer.cornerRadius = self.whiteView.width / 2.0;
            //取消计时器
            dispatch_source_cancel(self->_gcdTimer);
...
//图片输出完成
- (void)captureTool:(LSAVCaptureTool *)captureTool didOutputPhoto:(UIImage *)image error:(NSError *)error {
    [self.avCaptureTool stopRunning];
    NSLog(@"拍照结束");
    LSEditImageViewController *LSEditImageVC = [[LSEditImageViewController alloc] init];
    LSEditImageVC.image = image;
    LSEditImageVC.modalPresentationStyle = UIModalPresentationFullScreen;
    [self presentViewController:LSEditImageVC animated:NO completion:nil];
}

//音视频输出完成
- (void)captureTool:(LSAVCaptureTool *)captureTool didFinishRecordingToOutputFileAtURL:(NSURL *)outputFileURL error:(NSError *)error {
    [self.avCaptureTool stopRunning];
    NSLog(@"结束录制");
    LSEditVideoViewController *LSEditVideoVC = [[LSEditVideoViewController alloc] init];
    LSEditVideoVC.videoPath = outputFileURL;
    LSEditVideoVC.modalPresentationStyle = UIModalPresentationFullScreen;
    [self presentViewController:LSEditVideoVC animated:NO completion:nil];
}
