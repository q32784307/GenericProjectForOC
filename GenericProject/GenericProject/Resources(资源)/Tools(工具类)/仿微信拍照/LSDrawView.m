...
@interface LSDrawBezierPath : UIBezierPath

@property(nonatomic,strong)UIColor *color; //曲线颜色

@end

@implementation LSDrawBezierPath
...
/// 笔画
@property(nonatomic,strong)NSMutableArray <LSDrawBezierPath *>*lineArray;
/// 图层
@property(nonatomic,strong)NSMutableArray <CAShapeLayer *>*layerArray;
/// 删除的笔画
@property(nonatomic,strong)NSMutableArray <LSDrawBezierPath *>*deleteLineArray;
/// 删除的图层
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    if ([event allTouches].count == 1) {
        ...
        LSDrawBezierPath *path = [LSDrawBezierPath new];
        //2、移动画笔
...
- (void)touchesMoved:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    if (_isBegan || _isWork) {
        UITouch *touch = [touches anyObject];
        CGPoint point = [touch locationInView:self];
        LSDrawBezierPath *path = self.lineArray.lastObject;
...
//创建线条图层
- (CAShapeLayer *)createShapeLayer:(LSDrawBezierPath *)path {
...
- (void)setData:(NSDictionary *)data {
    ...
    if (lineArray.count) {
        for (LSDrawBezierPath *path in lineArray) {
            CAShapeLayer *slayer = [self createShapeLayer:path];
            [self.layer addSublayer:slayer];
            [self.layerArray addObject:slayer];
        }
