...
/// 涂鸦子菜单 画笔颜色选择
@interface LSSubmenuGraffitiView : UIView
...
@implementation  LSSubmenuGraffitiView
...
@interface LSSubmenuStickingCell : UICollectionViewCell

@property(nonatomic,strong)LSImageView *imageView;

@end

@implementation LSSubmenuStickingCell
...
- (void)setupUI {
    _imageView = [[LSImageView alloc] init];
...
- (void)setImage:(NSString *)imageName {
    ...
    _imageView.image = [LSImage imageNamed:imageName];
}

@end

/// 贴画子菜单
@interface LSSubmenuStickingView : UIView <UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout>

@property(nonatomic,strong)NSMutableArray *dataSource;
@property(nonatomic,strong)UICollectionView *collectionView;
@property(nonatomic,copy)void(^selectedImage)(LSImage *image); //选中的图片 贴画

@end

@implementation LSSubmenuStickingView
...
- (UICollectionView *)collectionView {
    if (_collectionView == nil) {
        ...
        [_collectionView registerClass:[LSSubmenuStickingCell class] forCellWithReuseIdentifier:@"ItemId"];
    }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    LSSubmenuStickingCell * item = [collectionView dequeueReusableCellWithReuseIdentifier:@"ItemId" forIndexPath:indexPath];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    self.selectedImage([LSImage imageNamed:self.dataSource[indexPath.row]]);
}
...
/// 图片马赛克 子菜单  马赛克类型选择
@interface LSSubmenuMosaicView : UIView
...
@implementation LSSubmenuMosaicView
...
@property(nonatomic,strong)LSSubmenuGraffitiView *submenuGraffiti; //涂鸦子菜单
@property(nonatomic,strong)LSSubmenuStickingView *submenuSticking; //贴图子菜单
@property(nonatomic,strong)LSSubmenuMosaicView *submenuMosaic;  //图片马赛克

@end

@implementation LSEditMenuView
...
- (void)setEditObject:(LSEditObject)editObject {
    _editObject = editObject;
    if (editObject == LSEditObjectPicture) {
        _menuTypes = @[@(LSEditMenuTypeGraffiti), @(LSEditMenuTypeSticking), @(LSEditMenuTypeText),@(LSEditMenuTypePictureMosaic), @(LSEditMenuTypePictureClipping)];
    ...
    }else if (editObject == LSEditObjectVideo) {
        _menuTypes = @[@(LSEditMenuTypeGraffiti), @(LSEditMenuTypeSticking), @(LSEditMenuTypeText), @(LSEditMenuTypeVideoClipping)];
...
#pragma mark - Getter
- (LSSubmenuGraffitiView *)submenuGraffiti {
    if (!_submenuGraffiti) {
        _submenuGraffiti = [[LSSubmenuGraffitiView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, 60)];
        _submenuGraffiti.hidden = YES;
        __weak typeof(self) weakSelf = self;
        _submenuGraffiti.selectedLineColor = ^(UIColor *lineColor) {
            weakSelf.selectEditMenu(LSEditMenuTypeGraffiti, @{@"lineColor":lineColor});
        };
        _submenuGraffiti.goBack = ^{
            weakSelf.selectEditMenu(LSEditMenuTypeGraffiti, @{@"goBack":@(YES)});
        };
    }
    return _submenuGraffiti;
}

- (LSSubmenuStickingView *)submenuSticking {
    if (!_submenuSticking) {
        _submenuSticking = [[LSSubmenuStickingView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, 60)];
        _submenuSticking.hidden = YES;
        __weak typeof(self) weakSelf = self;
        _submenuSticking.selectedImage = ^(LSImage *image) {
            weakSelf.selectEditMenu(LSEditMenuTypeSticking, @{@"image":image});
        };
    }
    return _submenuSticking;
}

- (LSSubmenuMosaicView *)submenuMosaic {
    if (!_submenuMosaic) {
        _submenuMosaic = [[LSSubmenuMosaicView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, 60)];
        _submenuMosaic.hidden = YES;
        __weak typeof(self) weakSelf = self;
        _submenuMosaic.selectedMosaicType = ^(NSInteger currentTypeIndex) {
            weakSelf.selectEditMenu(LSEditMenuTypePictureMosaic, @{@"mosaicType":@(currentTypeIndex)});
        };
        _submenuMosaic.goBack = ^{
            weakSelf.selectEditMenu(LSEditMenuTypePictureMosaic, @{@"goBack":@(YES)});
        };
    }
...
- (void)menuBtnClicked:(UIButton *)menuBtn {
    ...
    LSEditMenuType editMenuType = menuBtn.tag;
    switch (editMenuType) {
        ...
        case LSEditMenuTypeText:
            ...
            self.selectEditMenu(editMenuType, @{@"hidden":@(self.submenuSticking.hidden)});
            break;
        case LSEditMenuTypeVideoClipping:
            [self hiddenView:self.currentSubmenu hidden:YES];
