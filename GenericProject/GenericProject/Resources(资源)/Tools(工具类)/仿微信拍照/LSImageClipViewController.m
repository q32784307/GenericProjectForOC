...
@interface LSImageClipViewController ()<UIScrollViewDelegate, LSGridViewDelegate, LSImageZoomViewDelegate>

// 缩放视图
@property(nonatomic,strong)LSImageZoomView *zoomView;
//网格视图 裁剪框
@property(nonatomic,strong)LSGridView *gridView;
...
@implementation LSImageClipViewController
...
- (void)setupUI {
    ...
    self.maxGridRect = CGRectMake(KGridLRMargin, KGridTopMargin, self.view.width - KGridLRMargin * 2, self.view.height - KGridTopMargin - KGridBottomMargin- KBottomMenuHeight);
    
    CGSize newSize = CGSizeMake(self.view.width - 2 * KGridLRMargin, (self.view.width - 2 * KGridLRMargin) * self.image.size.height / self.image.size.width);
    if (newSize.height > self.maxGridRect.size.height) {
        ...
        self.zoomView.size = newSize;
        self.zoomView.y = KGridTopMargin;
        self.zoomView.centerX = self.view.width / 2.0;
    }else {
        self.zoomView.size = newSize;
        self.zoomView.center = CGPointMake(self.view.width / 2.0, (self.view.height - KBottomMenuHeight) / 2.0);
    }
...
- (LSImageZoomView *)zoomView {
    if (!_zoomView) {
        _zoomView = [[LSImageZoomView alloc] initWithFrame:CGRectMake(KGridLRMargin, KGridTopMargin, self.view.width - KGridLRMargin * 2,( self.view.width - KGridLRMargin * 2) * self.image.size.height / self.image.size.width)];
        _zoomView.centerY = (self.view.height - KBottomMenuHeight)/2.0;
...
- (LSGridView *)gridView {
    if (!_gridView) {
        _gridView = [[LSGridView alloc] initWithFrame:self.view.bounds];
...
- (UIButton *)rotateBtn {
    if (_rotateBtn == nil) {
        _rotateBtn = [[UIButton alloc] initWithFrame:CGRectMake(30, self.view.height - KBottomMenuHeight, 40, 30)];
...
- (UIButton *)cancleClipBtn {
    if (_cancleClipBtn == nil) {
        _cancleClipBtn = [[UIButton alloc] initWithFrame:CGRectMake(30, self.view.height - 30 - 20, 40, 30)];
...
- (UIButton *)recoveryBtn {
    if (_recoveryBtn == nil) {
        ...
        _recoveryBtn.centerX = self.view.width / 2.0;
        _recoveryBtn.centerY = self.cancleClipBtn.center.y;
...
- (UIButton *)doneClipBtn {
    if (_doneClipBtn == nil) {
        _doneClipBtn = [[UIButton alloc] initWithFrame:CGRectMake(self.view.width - 30 - 40, self.view.height - 30 - 20, 40, 30)];
...
- (void)rotateBtnClicked:(id)sender {
    ...
    //计算旋转之后
    CGSize newSize = CGSizeMake(self.view.width - 2 * KGridLRMargin, (self.view.width - 2 * KGridLRMargin) * height / width);
...
- (void)doneClipClicked:(id)sender {
    [self dismissViewControllerAnimated:NO completion:nil];
    UIImage *clipImage = [self.zoomView.imageView ls_imageByViewInRect:[self rectOfGridOnImageByGridRect:_gridView.gridRect]];
...
- (void)gridViewDidEndResizing:(LSGridView *)gridView {
    ...
    [UIView animateWithDuration:0.25
                     ...
                     animations:^{
        CGSize newSize = CGSizeMake(self.view.width - 2 * KGridLRMargin, (self.view.width - 2 * KGridLRMargin) * gridView.gridRect.size.height / gridView.gridRect.size.width);
        if (newSize.height > self.gridView.maxGridRect.size.height) {
        ...
        [self.zoomView setZoomScale:self.zoomView.zoomScale];
        // 调整contentOffset
        CGFloat zoomScale = self.zoomView.width / gridView.gridRect.size.width;
