...
/// 网格遮罩层  网格透明层
@interface LSGridMaskLayer : CAShapeLayer
...
@implementation LSGridMaskLayer
...
/// 网格层
@interface LSGridLayer : CAShapeLayer
///网格区域 默认CGRectZero
...
@class LSResizeControl;
@protocol LSResizeControlDelegate <NSObject>
/// 开始
- (void)resizeConrolDidBeginResizing:(LSResizeControl *)resizeConrol;
- (void)resizeConrolDidResizing:(LSResizeControl *)resizeConrol;
- (void)resizeConrolDidEndResizing:(LSResizeControl *)resizeConrol;

@end

/// 网格四边和四角控制大小的透明视图
@interface LSResizeControl : UIView
@property(weak,nonatomic)id<LSResizeControlDelegate> delegate;
...
@implementation LSResizeControl
...
//网格视图
@interface LSGridView ()<LSResizeControlDelegate>
@property(nonatomic,strong)LSGridLayer *gridLayer; //网格层
@property(nonatomic,strong)LSGridMaskLayer *gridMaskLayer; // 半透明遮罩层
@property(nonatomic,assign)CGRect initialRect; //高亮网格框的初始区域
//四个角
@property(nonatomic,strong)LSResizeControl *topLeftCornerView;
@property(nonatomic,strong)LSResizeControl *topRightCornerView;
@property(nonatomic,strong)LSResizeControl *bottomLeftCornerView;
@property(nonatomic,strong)LSResizeControl *bottomRightCornerView;
//四条边
@property(nonatomic,strong)LSResizeControl *topEdgeView;
@property(nonatomic,strong)LSResizeControl *leftEdgeView;
@property(nonatomic,strong)LSResizeControl *bottomEdgeView;
@property(nonatomic,strong)LSResizeControl *rightEdgeView;

@end

@implementation LSGridView
...
- (void)enableCornerViewUserInteraction:(UIView *)view {
    for (UIView *control in self.subviews) {
        if ([control isKindOfClass:[LSResizeControl class]]) {
            if (view) {
                if (control == view) {
...
- (LSResizeControl *)createResizeControl {
    LSResizeControl *control = [[LSResizeControl alloc] initWithFrame:(CGRect){CGPointZero, CGSizeMake(30, 30)}];
...
#pragma mark - Getter
- (LSGridMaskLayer *)gridMaskLayer {
    if (!_gridMaskLayer) {
        _gridMaskLayer = [[LSGridMaskLayer alloc] init];
...
- (LSGridLayer *)gridLayer {
    if (!_gridLayer) {
        _gridLayer = [[LSGridLayer alloc] init];
...
//返回正在调整网格大小时的网格区域
- (CGRect)cropRectMakeWithResizeControlView:(LSResizeControl *)resizeControlView {
    CGRect rect = self.gridRect;
