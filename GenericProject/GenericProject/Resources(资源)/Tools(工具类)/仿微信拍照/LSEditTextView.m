...
@interface LSEditTextView ()<UITextViewDelegate>
{
...
- (void)colorSelectionView:(CGFloat)keyboardHeight {
    ...
    for (int i = 0; i < count; i++) {
        UIButton * colorBtn = [[UIButton alloc] initWithFrame:CGRectMake(space + (itemSize.width + space)*i, self.height - keyboardHeight - 20 - 20, itemSize.width, itemSize.height)];
...
- (UIButton *)doneEditBtn {
    if (_doneEditBtn == nil) {
        _doneEditBtn = [[UIButton alloc] initWithFrame:CGRectMake(self.width - 50 - 15, 30, 40, 30)];
...
- (UILabel *)copyTextView:(UITextView *)textView {
    LSPaddingLabel *label = [[LSPaddingLabel alloc] initWithFrame:textView.bounds];
...
- (void)textViewDidChange:(UITextView *)textView {
    //最大高度
    CGFloat maxHeight = self.height - 100 - _keyboardHeight - 20 - 20 - 20;
    CGSize constraintSize = CGSizeMake(self.width - 2*30, MAXFLOAT);
    CGSize size = [textView sizeThatFits:constraintSize];
    if (size.height >= maxHeight) {
        textView.y = 100 - (size.height - maxHeight);
    } else {
        textView.y = 100;
    }
