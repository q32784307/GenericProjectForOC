...
typedef struct LSVideoClippingState LSVideoClippingState;

CG_INLINE LSVideoClippingState
LSVideoClippingStateMake(BOOL position, CGFloat value, UIGestureRecognizerState state)
{
    LSVideoClippingState videoClippingState;
...
/// 视频编辑裁剪选择框
@interface  LSVideoClippingBox : UIView
...
@property(nonatomic,copy)void(^changeClippingRange)(LSVideoClippingState clippingState);  //改变选择的裁剪区域

@end

@implementation LSVideoClippingBox
...
- (void)drawRect:(CGRect)rect {
    ...
    UIRectFill(rect);
    CGRect holeRectIntersection = CGRectIntersection(CGRectMake(self.leftTime.width + 10, 0, self.rightTime.x - (self.leftTime.x + 10), rect.size.height), rect);
...
- (void)setFrame:(CGRect)frame {
    ...
    self.boxInside.frame = CGRectMake(0, 0, self.width, self.height);
    self.leftTime.frame = CGRectMake(-10, 0, 10, self.height);
    self.rightTime.frame = CGRectMake(self.width, 0, 10, self.height);
}

#pragma mark - Getter
- (UIView *)boxInside {
    if (!_boxInside) {
        _boxInside = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.width, self.height)];
...
- (UILabel *)leftTime {
    if (!_leftTime) {
        _leftTime = [[UILabel alloc] initWithFrame:CGRectMake(-10, 0, 10, self.height)];
        _leftTime.text = @"||";
...
- (UILabel *)rightTime {
    if (!_rightTime) {
        _rightTime = [[UILabel alloc] initWithFrame:CGRectMake(self.width, 0, 10, self.height)];
        _rightTime.text = @"||";
        _rightTime.tag = 1;
...
- (void)changeClippingRange:(UIPanGestureRecognizer *)pan {
    ...
    if (pan.state == UIGestureRecognizerStateBegan) {
        self.changeClippingRange(LSVideoClippingStateMake(pan.view.tag, (pan.view.tag == 0 ? self.boxInside.x/self.width : self.rightTime.x/self.width), UIGestureRecognizerStateBegan));
    } else if (pan.state == UIGestureRecognizerStateChanged ) {
        if (pan.view == self.leftTime ) {
            //裁剪的视频时长必须>=1秒
            if (pan.view.x + transP.x < -10 || (self.rightTime.x - self.boxInside.x -  transP.x)/self.width * self.totalDuration < 1) {
                NSLog(@"视频超出了裁剪范围或裁剪后的时长小于1s");
            }else {
                self.boxInside.x = self.boxInside.x + transP.x;
                self.boxInside.width = self.boxInside.width - transP.x;
                pan.view.center = CGPointMake(pan.view.center.x + transP.x, pan.view.center.y);
            }
        }else if (pan.view == self.rightTime) {
            if (pan.view.x + transP.x > self.width || (self.rightTime.x + transP.x - self.boxInside.x) / self.width * self.totalDuration < 1) {
                NSLog(@"视频超出了裁剪范围或裁剪后的时长小于1s");
            }else {
                self.boxInside.width = self.boxInside.width + transP.x;
                pan.view.center = CGPointMake(pan.view.center.x + transP.x, pan.view.center.y);
        ...
        self.changeClippingRange(LSVideoClippingStateMake(pan.view.tag, (pan.view.tag == 0 ? self.boxInside.x / self.width : self.rightTime.x / self.width), UIGestureRecognizerStateChanged));
        ...
        if (pan.view == self.leftTime) {
            ...
            if (pan.view.x  > self.width) {
                pan.view.x = self.width;
            }
        }
        self.changeClippingRange(LSVideoClippingStateMake(pan.view.tag, (pan.view.tag == 0 ? self.boxInside.x
                                                                         / self.width : self.rightTime.x / self.width), UIGestureRecognizerStateEnded));
    }
}

@end

@interface LSEditVideoClipping ()
...
/// 视频裁剪框
@property(nonatomic,strong)LSVideoClippingBox *clippingBox; //剪辑选择框
...
@implementation LSEditVideoClipping
...
- (UIView *)contentView {
    if (!_contentView) {
        _contentView = [[UIView alloc] initWithFrame:CGRectMake(40, 10, self.width - 40 * 2, 50)];
...
- (UIView *)clippingBox {
    if (!_clippingBox) {
        _clippingBox = [[LSVideoClippingBox alloc] initWithFrame:self.contentView.frame];
        __weak typeof(self) weakSelf = self;
        _clippingBox.changeClippingRange = ^(LSVideoClippingState clippingState) {
...
- (UIButton *)cancleBtn {
    if (_cancleBtn == nil) {
        _cancleBtn = [[UIButton alloc] initWithFrame:CGRectMake(10, self.height - 10 - 30, 30, 30)];
...
- (UIButton *)doneBtn {
    if (_doneBtn == nil) {
        _doneBtn = [[UIButton alloc] initWithFrame:CGRectMake(self.width - 30 - 10, self.height - 10 - 30, 30, 30)];
...
- (void)generateImage {
    ...
    if (maxImageCount * maximumSize.width/[UIScreen mainScreen].scale < self.contentView.frame.size.width) {
        ...
        self.contentView.width = maxImageCount * maximumSize.width/[UIScreen mainScreen].scale;
        self.contentView.x = (self.width - self.contentView.width)/2.0;
        self.clippingBox.frame = self.contentView.frame;
        imageViewSize = CGSizeMake(self.contentView.height * maximumSize.width / maximumSize.height, self.contentView.height);
    }else {
        imageViewSize = CGSizeMake(self.contentView.width / maxImageCount, self.contentView.height);
    }
