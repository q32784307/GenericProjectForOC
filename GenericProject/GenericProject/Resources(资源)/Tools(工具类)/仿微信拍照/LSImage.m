...
@interface LSImage ()
{
    ...
    LSImageDecoder * _imageDecoder;  //解码工具
    NSArray <LSImageFrame *> * _preloadedFrames;  //预加载的图片帧
}
...
+ (LSImage *)imageNamed:(NSString *)name {
    ...
    NSArray *scales = [LSImage preferredScales];
...
+ (LSImage *)imageWithData:(NSData *)data scale:(CGFloat)scale {
    return [[self alloc] initWithData:data scale:scale];
}
...
- (instancetype)initWithData:(NSData *)data scale:(CGFloat)scale {
    ...
    @autoreleasepool {
        _imageDecoder = [[LSImageDecoder alloc] init];
...
- (void)setPreloadAllAnimatedImageFrames:(BOOL)preloadAllAnimatedImageFrames{
    ...
    if (_preloadAllAnimatedImageFrames && _imageDecoder.frameCount > 0) {
        ...
        for (NSUInteger i = 0, max = _imageDecoder.frameCount; i < max; i++) {
            LSImageFrame *imageFrame = [_imageDecoder imageFrameAtIndex:i];
            if (imageFrame) {
...
- (LSImageFrame *)imageFrameAtIndex:(NSInteger)index {
    ...
    dispatch_semaphore_wait(_preloadedLock, DISPATCH_TIME_FOREVER);
    LSImageFrame *imageFrame = _preloadedFrames[index];
    dispatch_semaphore_signal(_preloadedLock);
