...
@interface LSAVCaptureTool () <AVCapturePhotoCaptureDelegate, AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureAudioDataOutputSampleBufferDelegate>
...
@property(nonatomic,assign)LSAVCaptureType  avCaptureType; //音视频捕获类型 默认 LSAVCaptureTypeAv
...
@implementation LSAVCaptureTool

+ (instancetype)sharedAvCaptureTool {
    static LSAVCaptureTool *avCaptureTool = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        avCaptureTool = [[LSAVCaptureTool alloc] init];
    });
...
- (void)setOutputFileURL:(NSURL *)outputFileURL {
    ...
    if (self.avCaptureType == LSAVCaptureTypeAudio) {
        _assetWriter = [AVAssetWriter assetWriterWithURL:outputFileURL fileType:AVFileTypeAC3 error:nil];
    } else if (self.avCaptureType == LSAVCaptureTypeVideo || self.avCaptureType == LSAVCaptureTypeAv) {
...
//开始录制视频
- (void)startRecordVideoToOutputFileAtPath:(NSString *)path recordType:(LSAVCaptureType)avCaptureType{
    ...
    if ([self.assetWriter canAddInput:self.assetWriterAudioInput] && self.avCaptureType == LSAVCaptureTypeAv) {
...
- (void)startRecordAudioToOutputFileAtPath:(NSString *)path {
    self.avCaptureType = LSAVCaptureTypeAudio;
    //移除重复文件
    if ([[NSFileManager defaultManager] fileExistsAtPath:path]) {
        [[NSFileManager defaultManager] removeItemAtPath:path error:nil];
    }
...
- (void)writerVideoSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
    @autoreleasepool {
        @synchronized(self) {
            if (!self.canWrite && (self.avCaptureType == LSAVCaptureTypeAv || self.avCaptureType == LSAVCaptureTypeVideo)) {
...
- (void)writerAudioSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
    @autoreleasepool {
        @synchronized(self) {
            if (!self.canWrite && self.avCaptureType == LSAVCaptureTypeAudio) {
                [self.assetWriter startWriting];
                [self.assetWriter startSessionAtSourceTime:CMSampleBufferGetPresentationTimeStamp(sampleBuffer)];
                self.canWrite = YES;
            }
            if (self.assetWriterAudioInput.readyForMoreMediaData && self.canWrite) {
                ...
                if (!success) {
                    @synchronized (self) {
                        ...
                        }else if (self.avCaptureType == LSAVCaptureTypeAv || self.avCaptureType == LSAVCaptureTypeVideo) {
                            [self stopRecordVideo];
                        }
                    }
                }
