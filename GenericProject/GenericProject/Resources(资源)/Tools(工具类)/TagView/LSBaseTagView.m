...
static CGFloat LSLineViewHeight = 1;

@interface LSBaseTagView ()

@property(nonatomic,strong)LSCustomButton *selectedBtn;

@end

@implementation LSBaseTagView
...
- (void)setTagArray:(NSArray *)tagArray {
    ...
    LSCustomButton * lastRowButton;
    for (int i = 0; i < tagArray.count; i++) {
        LSBaseTagViewModel *tagModel = tagArray[i];
        LSCustomButton * button = [LSCustomButton buttonWithType:UIButtonTypeCustom];
        ...
        if (self.cornerRadius >0) {
            button.layer.borderWidth = LSLineViewHeight;
        }
        ...
        if (tittleImage) {
            ...
            button.ls_spacing = self.titleImageSpae >0 ?self.titleImageSpae:10;
            button.ls_buttonType = self.buttonType;
        }else{
            // 没有图片的话 默认无图片
            button.ls_buttonType = LSCustomButtonNoHaveImageLabel;
        }
...
- (void)buttonClick:(LSCustomButton *)btn {
    btn.selected = !btn.selected;
    LSBaseTagViewModel * model = self.tagArray[btn.tag - buttonStartTag];
    ...
    if (_isMultiSelect) {
        ...
        if (self.selectedBtn && self.selectedBtn != btn) {
            ...
            LSBaseTagViewModel * lastSelectedModel = self.tagArray[self.selectedBtn.tag - buttonStartTag];
...
- (void)changeBtnStatus:(LSCustomButton *)btn {
    if (btn.selected) {
        btn.backgroundColor = self.selectedBgColor?:self.bgColor;
        btn.layer.borderWidth = LSLineViewHeight;
...
- (NSArray *)getSelectDataArray {
    ...
    for (LSBaseTagViewModel * model in self.tagArray) {
        if (model.isSelected) {
