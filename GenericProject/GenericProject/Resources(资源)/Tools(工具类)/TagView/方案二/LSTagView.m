...
@interface LSTagView ()<UIScrollViewDelegate>
...
- (void)addTagWithModel:(id)model {
    ...
    LSTagItem *item = [[LSTagItem alloc]initWithFrame:CGRectMake(0, 0, 28, 28)];
    item.model = model;
...
- (void)removeTag:(id)model {
    ...
    for (LSTagItem *item in self.scrollView.subviews) {
