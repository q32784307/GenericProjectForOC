...
#import "NetworkAccessibity.h"

NSString * const NetworkAccessibityChangedNotification = @"NetworkAccessibityChangedNotification";
...
@implementation NetworkAccessibity
...
+ (NetworkAccessibity *)sharedInstance {
    static NetworkAccessibity * instance = nil;
...
static void ReachabilityCallback(SCNetworkReachabilityRef target, SCNetworkReachabilityFlags flags, void* info) {
    NetworkAccessibity *networkAccessibity = (__bridge NetworkAccessibity *)info;
    if (![networkAccessibity isKindOfClass: [NetworkAccessibity class]]) {
        return;
    }
...
- (void)startCheck {
    if ([UIDevice currentDevice].systemVersion.floatValue < 10.0 || [self currentReachable]) {
        ...
        /* 先用 currentReachable 判断，若返回的为 YES 则说明：
         ...
         此时是有网络访问权限的，直接返回 NetworkAccessible
         **/
...
- (void)notiWithAccessibleState:(NetworkAccessibleState)state {
    ...
    if (state != _previousState) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:NetworkAccessibityChangedNotification object:nil];
    }
}
