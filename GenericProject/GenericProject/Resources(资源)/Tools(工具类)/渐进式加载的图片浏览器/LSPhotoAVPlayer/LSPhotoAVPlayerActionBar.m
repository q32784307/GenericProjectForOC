...
@interface LSPhotoAVPlayerSlider : UISlider

@end

@implementation LSPhotoAVPlayerSlider

- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        [self setThumbImage:[UIImage imageNamed:@"LSPhotoBrowser.bundle/circlePoint@3x.png"] forState:UIControlStateNormal];
...
@interface LSPhotoAVPlayerActionBar ()
{
    BOOL _isDragging;
}
...
@property(nonatomic,strong)LSPhotoAVPlayerSlider *slider;

@end

@implementation LSPhotoAVPlayerActionBar

/****************************** == lazy == ********************************/
- (UIButton *)pauseStopBtn {
    if (!_pauseStopBtn) {
        _pauseStopBtn = [UIButton buttonWithType:UIButtonTypeCustom];
        [_pauseStopBtn setImage:[UIImage imageNamed:@"LSPhotoBrowser.bundle/pause@2x.png"] forState:UIControlStateNormal];
...
- (LSPhotoAVPlayerSlider *)slider {
    if (!_slider) {
        _slider = [[LSPhotoAVPlayerSlider alloc] init];
...
- (void)actionBarSliderFinished:(LSPhotoAVPlayerSlider *)slider {
...
- (void)actionBarSliderDown:(LSPhotoAVPlayerSlider *)slider {
    _isDragging = true;
...
- (void)setIsPlaying:(BOOL)isPlaying {
    ...
    if (isPlaying) {
        ...
        [_pauseStopBtn setImage:[UIImage imageNamed:@"LSPhotoBrowser.bundle/play@2x.png"] forState:UIControlStateNormal];
    }
}
