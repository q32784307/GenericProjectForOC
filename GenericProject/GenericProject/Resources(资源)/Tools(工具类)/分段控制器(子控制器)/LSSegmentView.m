...
typedef void(^LSIndexBlock)(NSUInteger ,UIButton *);
int const LSSegmentHeight = 44;
...
@property(nonatomic,copy)LSIndexBlock indexBlock;
...
- (void)segmentBasicSetting {
    ...
    _segmentHeight = LSSegmentHeight;
    _minItemSpace = 20.;
...
- (void)scrollIndicateView {
    ...
    [UIView animateWithDuration:_duration delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        if (_style == LSSegmentStyleDefault) {
...
- (void)adjustOffsetXToFixIndicatePosition:(CGFloat)offsetX {
    ...
    CGFloat originMovedX = _style == LSSegmentStyleDefault? (CGRectGetMinX(_selectedButton.frame) + _buttonSpace) : CGRectGetMinX(_selectedButton.frame);
    CGFloat targetMovedWidth = [self widthAtIndex:currentIndex];//需要移动的距离
    
    CGFloat targetButtonWidth = _style == LSSegmentStyleDefault? ([self widthAtIndex:buttonIndex] - 2 * _buttonSpace) : [self widthAtIndex:buttonIndex]; // 这个会影响width
    CGFloat originButtonWidth = _style == LSSegmentStyleDefault? ([self widthAtIndex:[self index]] - 2 * _buttonSpace) : [self widthAtIndex:[self index]];
...
- (void)setStyle:(LSSegmentStyle)style {
    _style = style;
    
    if (LSSegmentStyleFlush) {
