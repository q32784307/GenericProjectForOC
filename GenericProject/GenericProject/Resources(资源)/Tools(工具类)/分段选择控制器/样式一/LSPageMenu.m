...
@interface LSPageMenuScrollView : UIScrollView

@end

@implementation LSPageMenuScrollView
...
@interface LSPageMenuLine : UIImageView

@property (nonatomic, copy) void(^hideBlock)(void);

@end

@implementation LSPageMenuLine
...
@interface LSPageMenuButton : UIButton

- (instancetype)initWithImagePosition:(LSItemImagePosition)imagePosition;

@property (nonatomic) LSItemImagePosition imagePosition; // 图片位置
@property (nonatomic, assign) CGFloat imageTitleSpace; // 图片和文字之间的间距

@end

@implementation LSPageMenuButton
...
- (void)initialize {
    _imagePosition = LSItemImagePositionLeft;
...
- (CGRect)titleRectForContentRect:(CGRect)contentRect {
    ...
    switch (self.imagePosition) {
        case LSItemImagePositionLeft:
        case LSItemImagePositionDefault: {
...
- (void)setImagePosition:(LSItemImagePosition)imagePosition {
...
@interface LSPageMenu()

@property (nonatomic, assign) LSPageMenuTrackerStyle trackerStyle;
...
@property (nonatomic, weak) LSPageMenuScrollView *itemScrollView;
@property (nonatomic, weak) LSPageMenuButton *functionButton;
@property (nonatomic, strong) NSMutableArray *buttons;
@property (nonatomic, strong) LSPageMenuButton *selectedButton;
...
@implementation LSPageMenu

#pragma mark - public
+ (instancetype)pageMenuWithFrame:(CGRect)frame trackerStyle:(LSPageMenuTrackerStyle)trackerStyle {
    LSPageMenu *pageMenu = [[LSPageMenu alloc] initWithFrame:frame trackerStyle:trackerStyle];
    return pageMenu;
}

- (instancetype)initWithFrame:(CGRect)frame trackerStyle:(LSPageMenuTrackerStyle)trackerStyle {
    if (self = [super init]) {
        self.frame = frame;
...
- (void)setItems:(NSArray *)items selectedItemIndex:(NSInteger)selectedItemIndex {
    ...
    if (self.buttons.count) {
        for (LSPageMenuButton *button in self.buttons) {
    ...
    if (self.buttons.count) {
        ...
        LSPageMenuButton *selectedButton = [self.buttons objectAtIndex:selectedItemIndex];
...
- (void)insertItem:(LSPageMenuButtonItem *)item atIndex:(NSUInteger)itemIndex animated:(BOOL)animated {
    self.insert = YES;
    NSMutableArray *objects = self.items.mutableCopy;
...
- (void)removeItemAtIndex:(NSUInteger)itemIndex animated:(BOOL)animated {
    ...
    if (itemIndex < self.buttons.count) {
        LSPageMenuButton *button = [self.buttons objectAtIndex:itemIndex];
...
- (void)removeAllItems {
    ...
    for (int i = 0; i < self.buttons.count; i++) {
        LSPageMenuButton *button = self.buttons[i];
        [button removeFromSuperview];
    }
...
- (void)setItem:(LSPageMenuButtonItem *)item forItemIndex:(NSUInteger)itemIndex {
    if (item == nil) return;
    if (itemIndex < self.buttons.count) {
        LSPageMenuButton *button = [self.buttons objectAtIndex:itemIndex];
...
- (void)setItem:(LSPageMenuButtonItem *)item forItemAtIndex:(NSUInteger)itemIndex {
    [self setItem:item forItemIndex:itemIndex];
}

- (LSPageMenuButtonItem *)itemAtIndex:(NSUInteger)itemIndex {
    if (itemIndex < self.items.count) {
        id object = [self.items objectAtIndex:itemIndex];
        return object;
    }
...
- (void)setContent:(id)content forItemIndex:(NSUInteger)itemIndex {
    ...
    if (itemIndex < self.buttons.count) {
        ...
        } else if ([content isKindOfClass:[LSPageMenuButtonItem class]]) {
            LSPageMenuButtonItem *item = (LSPageMenuButtonItem *)content;
            [button setTitle:item.title forState:UIControlStateNormal];
            [button setImage:item.image forState:UIControlStateNormal];
            button.imagePosition = item.imagePosition;
            button.imageTitleSpace = item.imageTitleSpace;
...
- (void)setFunctionButtonWithItem:(LSPageMenuButtonItem *)item forState:(UIControlState)state {
...
- (void)setTitle:(nullable NSString *)title image:(nullable UIImage *)image imagePosition:(LSItemImagePosition)imagePosition imageRatio:(CGFloat)ratio forItemIndex:(NSUInteger)itemIndex {
    if (itemIndex < self.buttons.count) {
        LSPageMenuButton *button = [self.buttons objectAtIndex:itemIndex];
        [button setTitle:title forState:UIControlStateNormal];
        [button setImage:image forState:UIControlStateNormal];
...
- (void)setTitle:(nullable NSString *)title image:(nullable UIImage *)image imagePosition:(LSItemImagePosition)imagePosition imageRatio:(CGFloat)ratio imageTitleSpace:(CGFloat)imageTitleSpace forItemIndex:(NSUInteger)itemIndex {
    if (itemIndex < self.buttons.count) {
        LSPageMenuButton *button = [self.buttons objectAtIndex:itemIndex];
        [button setTitle:title forState:UIControlStateNormal];
        [button setImage:image forState:UIControlStateNormal];
...
- (void)setFunctionButtonTitle:(nullable NSString *)title image:(nullable UIImage *)image imagePosition:(LSItemImagePosition)imagePosition imageRatio:(CGFloat)ratio forState:(UIControlState)state {
...
- (void)setFunctionButtonTitle:(nullable NSString *)title image:(nullable UIImage *)image imagePosition:(LSItemImagePosition)imagePosition imageRatio:(CGFloat)ratio imageTitleSpace:(CGFloat)imageTitleSpace forState:(UIControlState)state {
    [self.functionButton setTitle:title forState:state];
    [self.functionButton setImage:image forState:state];
    self.functionButton.imagePosition = imagePosition;
...
- (void)addButton:(NSInteger)index object:(id)object animated:(BOOL)animated {
    ...
    LSPageMenuButton *button = [LSPageMenuButton buttonWithType:UIButtonTypeCustom];
    ...
    if ([object isKindOfClass:[NSString class]]) {
        ...
        LSPageMenuButtonItem *item = (LSPageMenuButtonItem *)object;
        [button setTitle:item.title forState:UIControlStateNormal];
        [button setImage:item.image forState:UIControlStateNormal];
        button.imagePosition = item.imagePosition;
        button.imageTitleSpace = item.imageTitleSpace;
...
- (BOOL)haveOrNeedsTracker {
    if (self.trackerStyle != LSPageMenuTrackerStyleTextZoom && self.trackerStyle != LSPageMenuTrackerStyleNothing) {
        return YES;
    }
    return NO;
}
...
- (void)setupSubViews {
    // 必须先添加分割线
    LSPageMenuLine *dividingLine = [[LSPageMenuLine alloc] init];
    ...
    LSPageMenuScrollView *itemScrollView = [[LSPageMenuScrollView alloc] init];
    ...
    LSPageMenuButton *functionButton = [LSPageMenuButton buttonWithType:UIButtonTypeCustom];
...
// 按钮点击方法
- (void)buttonInPageMenuClicked:(LSPageMenuButton *)sender {
    NSInteger fromIndex = self.selectedButton ? self.selectedButton.tag-100 : sender.tag - 100;
    NSInteger toIndex = sender.tag - 100;
    ...
    if (self.selectedButton != sender) {
        ...
        if (self.trackerStyle == LSPageMenuTrackerStyleTextZoom || _selectedItemZoomScale != 1) {
            if (labs(toIndex-fromIndex) >= 2) { // 该条件意思是当外界滑动scrollView连续的滑动了超过2页
                for (LSPageMenuButton *button in self.buttons) { // 必须遍历将非选中按钮还原缩放，而不是仅仅只让上一个选中的按钮还原缩放。因为当用户快速滑动外界scrollView时，会频繁的调用-zoomForTitleWithProgress:fromButton:toButton:方法，有可能经过的某一个button还没彻底还原缩放就直接过去了，从而可能会导致该按钮文字会显示不全，所以在这里，将所有非选中的按钮还原缩放
...
- (void)moveItemScrollViewWithSelectedButton:(LSPageMenuButton *)selectedButton {
...
// 移动跟踪器
- (void)moveTrackerWithSelectedButton:(LSPageMenuButton *)selectedButton {
...
// 功能按钮的点击方法
- (void)functionButtonClicked:(LSPageMenuButton *)sender {
...
- (void)prepareMoveTrackerFollowScrollView:(UIScrollView *)scrollView {
    ...
    switch (self.trackerFollowingMode) {
        case LSPageMenuTrackerFollowingModeAlways:
            // 这个方法才开始移动跟踪器
            ...
            break;
        case LSPageMenuTrackerFollowingModeHalf:{
            LSPageMenuButton *fromButton;
            LSPageMenuButton *toButton;
            if (progress > 0.5) {
...
- (void)moveTrackerWithProgress:(CGFloat)progress fromIndex:(NSInteger)fromIndex toIndex:(NSInteger)toIndex currentOffsetX:(CGFloat)currentOffsetX beginOffsetX:(CGFloat)beginOffsetX {
    ...
    if (self.trackerStyle == LSPageMenuTrackerStyleLine) {
    ...
    } else if (self.trackerStyle == LSPageMenuTrackerStyleLineAttachment) {
    ...
    } else if (self.trackerStyle == LSPageMenuTrackerStyleTextZoom || self.trackerStyle == LSPageMenuTrackerStyleNothing) {
        // 缩放文字
        if (_selectedItemZoomScale != 1) {
            [self zoomForTitleWithProgress:progress fromButton:fromButton toButton:toButton];
        }
    } else if (self.trackerStyle == LSPageMenuTrackerStyleRoundedRect) {
        newCenter.x = fromButton.center.x + xDistance * progress;
...
- (void)setBridgeScrollView:(UIScrollView *)bridgeScrollView {
    ...
    if (_bridgeScrollView && bridgeScrollView != _bridgeScrollView) {
        [_bridgeScrollView removeObserver:self forKeyPath:@"contentOffset"];
    };
    _bridgeScrollView = bridgeScrollView;
    if (bridgeScrollView) {
        [bridgeScrollView addObserver:self forKeyPath:@"contentOffset" options:NSKeyValueObservingOptionNew context:nil];
    }
}

- (void)setTrackerStyle:(LSPageMenuTrackerStyle)trackerStyle {
    _trackerStyle = trackerStyle;
    switch (trackerStyle) {
        case LSPageMenuTrackerStyleLine:
        case LSPageMenuTrackerStyleLineLongerThanItem:
        case LSPageMenuTrackerStyleLineAttachment:
            self.tracker.backgroundColor = _selectedItemTitleColor;
            break;
        case LSPageMenuTrackerStyleRoundedRect:
        case LSPageMenuTrackerStyleRect:
            ...
            _trackerHeight = 0;
            break;
        case LSPageMenuTrackerStyleTextZoom:
            // 此样式下默认1.3
...
- (void)setSelectedItemIndex:(NSInteger)selectedItemIndex {
    _selectedItemIndex = selectedItemIndex;
    if (self.buttons.count) {
        LSPageMenuButton *button = [self.buttons objectAtIndex:selectedItemIndex];
        [self buttonInPageMenuClicked:button];
    }
}

- (void)setDelegate:(id<LSPageMenuDelegate>)delegate {
    if (delegate == _delegate) {return;}
    _delegate = delegate;
    if (self.buttons.count) {
        LSPageMenuButton *button = [self.buttons objectAtIndex:_selectedItemIndex];
        [self delegatePerformMethodWithFromIndex:button.tag-100 toIndex:button.tag-100];
...
- (void)setPermutationWay:(LSPageMenuPermutationWay)permutationWay {
    _permutationWay = permutationWay;
    if (!_forceUseSettingSpacing) {
        if (_permutationWay == LSPageMenuPermutationWayNotScrollEqualWidths) {
...
- (void)setCloseTrackerFollowingMode:(BOOL)closeTrackerFollowingMode {
    ...
    if (closeTrackerFollowingMode) {
        self.trackerFollowingMode = LSPageMenuTrackerFollowingModeEnd;
    } else {
        self.trackerFollowingMode = LSPageMenuTrackerFollowingModeAlways;
    }
}
#pragma mark - getter
- (NSArray *)items {
...
- (void)layoutSubviews {
    ...
    for (int i= 0 ; i < self.buttons.count; i++) {
        ...
        } else if (button.currentTitle.length && button.currentImage && (button.imagePosition == LSItemImagePositionRight || button.imagePosition == LSItemImagePositionLeft || button.imagePosition == LSItemImagePositionDefault)) {
            contentW = textW + imageW + button.imageTitleSpace+button.contentEdgeInsets.left+button.contentEdgeInsets.right;
        } else if (button.currentTitle && button.currentImage && (button.imagePosition == LSItemImagePositionTop || button.imagePosition == LSItemImagePositionBottom)) {
    ...
    if (self.permutationWay == LSPageMenuPermutationWayNotScrollAdaptContent && diff < 0) {
    ...
    [self.buttons enumerateObjectsUsingBlock:^(LSPageMenuButton *button, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (self.permutationWay == LSPageMenuPermutationWayScrollAdaptContent) {
        ...
        } else if (self.permutationWay == LSPageMenuPermutationWayNotScrollEqualWidths) {
            // 求出外界设置的按钮宽度之和
...
- (void)resetupTrackerFrameWithSelectedButton:(LSPageMenuButton *)selectedButton {
    CGFloat trackerX;
    CGFloat trackerY;
    CGFloat trackerW;
    CGFloat trackerH;
...
@implementation LSPageMenuButtonItem

+ (instancetype)itemWithTitle:(NSString *)title image:(UIImage *)image {
    LSPageMenuButtonItem *item = [[LSPageMenuButtonItem alloc] initWithTitle:title image:image imagePosition:LSItemImagePositionDefault];
    return item;
}

+ (instancetype)itemWithTitle:(NSString *)title image:(UIImage *)image imagePosition:(LSItemImagePosition)imagePosition {
    LSPageMenuButtonItem *item = [[LSPageMenuButtonItem alloc] initWithTitle:title image:image imagePosition:imagePosition];
    return item;
}

- (instancetype)initWithTitle:(NSString *)title image:(UIImage *)image imagePosition:(LSItemImagePosition)imagePosition {
    if (self = [super init]) {
        self.title = title;
        self.image = image;
        self.imagePosition = imagePosition;
